// Gjoa semantic platform layer — top-level namespace.
//
// One Gjoa per chrome window. Established at chrome-window init,
// stored on `window.Gjoa` (and `window.gjoaTest.Gjoa` for tests).
//
// Architectural sentence (from docs/dev/firefox-upstream-stability.md):
//
// > Gjoa live state is window-scoped because Firefox live state is
// > window-scoped. Gjoa persisted state is global because Gjoa
// > sessions, checkpoints, and history are user-level concepts. Firefox
// > events invalidate Gjoa state; Gjoa reconcilers rebuild
...
//   - WindowTabs API — `Gjoa.windows.current().tabs.{list,selected,pin,unpin,close,…}`.
//
// Not yet wired (deferred to the roadmap):
//   - Persisted APIs (Gjoa.sessions / .history / .checkpoints) — M5/M6.
//   - Multi-instance scope params + instance discovery — M11.
//   - Cross-window aggregation (Gjoa.tabs.findAcrossWindows) — M12.
...
import { makeGjoaWindow, type GjoaWindow } from "./window.ts";
...
export type GjoaAPI = {
  ...
  windows: { current(): GjoaWindow };
  /** Cross-window tab aggregator. `Gjoa.tabs.all()` returns every tab
  ...
  flush(): Promise<void>;
  /** Diagnostic — scheduler state. Exposed via `gjoaTest.Gjoa.diag()`. */
  diag(): {
...
export function makeGjoa(deps: GjoaDeps): GjoaAPI {
  const scheduler = makeScheduler();
  const tabsReconciler: TabsReconcilerAPI = makeTabsReconciler({ scheduler });
  const win: GjoaWindow = makeGjoaWindow(scheduler);
