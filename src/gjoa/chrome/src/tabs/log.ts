// Debug logging — writes a timestamped event log to <profile>/gjoa-debug.log
// when the gjoa.debug pref is true. Lines also go to console.log.
//
...
/**
 * Tagged log function. Returns a no-op when gjoa.debug is false (cheap to call).
...
const LOG_FILENAME = "gjoa-debug.log";
...
export function createLogger(tag: string): Logger {
  const consolePrefix = `[GJOA:${tag}]`;
  return (event, data = {}) => {
    if (!Services.prefs.getBoolPref("gjoa.debug", false)) return;
    maybeRotate();
