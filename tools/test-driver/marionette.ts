...
export interface MarionetteClient {
  ...
  /** WebDriver:TakeScreenshot — base64 PNG of the active window. */
...
async function initClient(socket: Socket): Promise<MarionetteClient> {
  ...
  return {
    ...
    async takeScreenshot() {
      const r = await send<string | { value: string }>("WebDriver:TakeScreenshot", {
        full: true,
        hash: false,
      });
      return typeof r === "string" ? r : r.value;
